...
- `plotly::export()` function allows exporting an SVG vector image using the RSelenium package to establish a connection to a web browser

```{r plotly-export-svg, eval=TRUE, message=FALSE, echo=FALSE}
svg_file <- "plotly-multiple-y.svg"
svg_fromfile <- file.path("~/Downloads", svg_file)
svg_tofile <- file.path("assets/img", svg_file)
...
rD[["server"]]$stop()
## move saved file
file.copy(from = svg_fromfile, to = svg_tofile)
unlink(svg_fromfile)
```

```{r plotly-include-svg, out.width='100%', echo=FALSE, eval=TRUE}
